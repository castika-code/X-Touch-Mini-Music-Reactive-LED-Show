#!/bin/bash
# Removes the launchd agent and stops the show. The folder itself, XTouchShow.app
# included, is left untouched. To switch autostart back on, run: bash install.sh
...
echo "Done. To switch autostart back on, run: bash install.sh"
echo "Delete the whole folder to remove everything else."
